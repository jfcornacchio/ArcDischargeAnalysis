...
if fig
    if dim == 2
        % Plot boundary edges and point set. The input matrix is in the
        % form [rows cols], so the first column is really the "Y" and the
        % second column is the "X"
        y = X(:,1);
        x = X(:,2);
%         plot(y(B)',x(B)','r','linewidth',2), hold on
...
if nargout == 2    
    ...
    S = struct('tri',imap(T),'vol',vol,'rcc',rcc,'bnd',imap(B),'XCoord',x(B(:)),'YCoord',y(B(:)));
end
